...
static FODSELSNUMMER_PATTERN: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"\b\d{6}-?\d{5}\b").expect("Invalid fødselsnummer regex pattern")
});
...
mod tests {
    ...
    fn test_valid_fodselsnummer() {
        ...
        assert!(FodselsnummerDetector::validate_fodselsnummer("150765-00565")); // With dash
    }
    
    #[test]
    fn test_invalid_fodselsnummer_checksum() {
        assert!(!FodselsnummerDetector::validate_fodselsnummer("15076500566")); // Wrong K2
    }

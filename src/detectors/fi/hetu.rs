...
static HETU_PATTERN: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"\b\d{6}[+\-ABCDEFHJKLMNPRSTUVWXY]\d{3}[0-9A-Y]\b").expect("Invalid HETU regex pattern")
});
...
impl HetuDetector {
    ...
    fn validate_date(hetu: &str) -> bool {
        ...
        if !['+', '-', 'A', 'B', 'C', 'D', 'E', 'F', 'H', 'J', 'K', 'L', 'M', 'N', 'P', 'R', 'S', 'T', 'U', 'V', 'W', 'X', 'Y'].contains(&century_char) {
            return false;
        }
